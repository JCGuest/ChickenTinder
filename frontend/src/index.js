document.addEventListener('DOMContentLoaded', () => {
  ...
  btnNext.addEventListener('click', function nex() {
    ...
    if (!what.value) {
      alert(
        'Search term can not be blank. Use whatever you would type in to Yelp.'
      );
    } else if (!where.value) {
      ...
      fetch(`${HOME}/users/create?&user[name]=${user}`, userConfig)
        ...
        .then((json) => {
          ...
          if (errs) {
            errs.forEach((e) => {
              alert(
                `Name ${e}. Please click "New here?" again to enter a new name.`
              );
            });
          } else {
            alert(
              `You have created username: ${json['data']['attributes']['name']}.\nPlease remember this for the player entry after you have entered your search.`
            );
          }
        });
    }
    ...
    playBtn.addEventListener('click', function playFunc() {
      ...
      if (invalid[0]) {
        ...
        for (let i = 1; i <= NUMPLAYERS; i++) {
          ...
          fetch(
            `${HOME}/users/login?&user[name]=${name}&user[game_id]=${GAMEID}`,
            userConfig
          )
            .then((response) => {
  ...
  function yelpRender(i, player) {
    ...
    function like(e) {
      ...
      fetch(
        `${HOME}/likes?username=${USERARY[player - 1]}&name=${
          result.name
        }&yelp_id=${result.id}`,
        likeConfig
      )
  ...
  function megaList(allLikes) {
    ...
    if (names[0]) {
      ...
      megaH2.innerHTML = `A list of all businesses ${nameParse(USERARY)} and ${
        USERARY.slice(-1)[0]
      } have all liked on Chicken Tinder... `;
